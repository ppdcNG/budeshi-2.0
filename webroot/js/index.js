...
$(document).ready(function () {
    ...
    UIkit.upload('.document-upload', {
        ...
        error: function (e) {
            UIkit.notification("File uploade failed", { status: "success", timeout: 2000 });
        },
        complete: function (e) {
...
function addDocument() {
    let required_fields = ["doc-title", "doc-description"];
    var valid = validateFields(required_fields);
    if (valid != true) {
        UIkit.notification("Missing Required Field " + valid, { status: 'danger', timeout: 3000 });
        return;
    ...
    showDocument(num, doc_obj.title, doc_obj.documentType, doc_obj.format, doc_obj.uri);
}
...
function removeDocument(id, document) {
    var to_url = abs_path + "release/delajaxdocument";
    ...
    $("#documents-container").append(`<div id='` + id + `'>
        ...
        type + `
                                ...
                                <div class="uk-card-footer">
                                    ...
                                    <a href="#" id='remove-ducument' title="Delete Document" uk-tooltip="pos: bottom" class='uk-float-right' onclick="removeDocument('`+ id + `','` + document + `')"
