...
var items = [];
var tenderers = [];
var require_fields = ["tender-title", "tender-desc", "id","mainProcurementCategory"];
...
$("#edit-release").click(function () {
    ...
    if(items.length < 1){
        UIkit.notification("Must Add Tender Items Before Save " + valid, { status: 'danger', timeout: 3000 });
        return;
    }
    console.log("blah");
    var release = form2js("release-form", ".",false);
    ...
    release.tender.milestones = milestones;
    release.tender.items = items;
    release.tender.documents = documents
    release.milestone = undefined;
    var value = JSON.stringify(release);
    console.log(value);
    var id = $("#project_id").val();
